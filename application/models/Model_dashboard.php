...
class Model_dashboard extends CI_Model {
    ...
    {
        $this->db->select('id, COUNT(event) as event, date_event');
        $this->db->group_by('date_event');
        $this->db->order_by('id');
        return $this->db->get('list_sp');
    }
    
    public function get_list_schedule($date_update)
    {
        $this->db->select('list_sp.*, user.username');
        $this->db->join('user', 'user.id = list_sp.id_user', 'left');
        $this->db->where('list_sp.date_event', $date_update);
        $this->db->order_by('list_sp.id', 'desc');
        return $this->db->get('list_sp')->result_array();
    }
